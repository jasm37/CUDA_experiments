...
int main(int argc, char **argv)
{
    ...
    float *d_ker_rho, *d_ker_sigma, *d_conv, *d_imgIn, *d_imgOut, *d_grad, *d_dx_conv, *d_dy_conv, *d_coeff1, *d_coeff2, *d_coeff3;
    ...
    cudaMalloc(&d_G, coeff_size);CUDA_CHECK;
    cudaMemset(d_G, 0, coeff_size);
    ...
    for(int it = 0; it < nit; it++)
    {
        ...
        // 1b., c. Compute matrix G
        compute_G<<<grid, block>>>(d_G, dd_coeff1, dd_coeff2, dd_coeff3, alpha, cG, w, h, nc);
        // 2. Compute arguments of the divergence function : G*grad(u)
