...
__device__
void set_shared(float *d_img, float *sh_data, int dimx, int dimy, int nc, int rad)
{
    int bx = blockDim.x, by = blockDim.y;
    int shh = blockDim.x + 2 * rad, shw = blockDim.y + 2 * rad;
    int mx = bx * blockIdx.x -rad, my = by * blockIdx.y -rad;
    int t = threadIdx.x + threadIdx.y * blockDim.x;
    int sh_x, sh_y;
    for (int n = 0; n < nc; n++)
    {
        for(int i = t; i < shh * shw; i = i + bx*by )
        {
            sh_x = i % shw; sh_y = i / shw;
            sh_data[i] = d_img[(mx+sh_x) + (my+sh_y)*dimx + dimx * dimy * n];
        }
    }
    __syncthreads();
...
void do_GPUconvolution(float *conv, float *ker, float *a, int r, int dimx, int dimy, int nc)
{
    ...
    //set_shared(a, sh_data, dimx, dimy, nc, r);
    ///
    /*
    int bx = blockDim.x, by = blockDim.y;
    int shh = blockDim.x + 2 * r, shw = blockDim.y + 2 * r;
    int mx = bx * blockIdx.x -r, my = by * blockIdx.y -r;
    int t = threadIdx.x + threadIdx.y * blockDim.x;
    int sh_x, sh_y;
    for (int n = 0; n < nc; n++)
    {
        for(int ind = t; ind < shh * shw; ind = ind + bx*by )
        {
            sh_x = ind % shw; sh_y = ind / shw;
            sh_data[ind] = a[(mx+sh_x) + (my+sh_y)*dimx + dimx * dimy * n];
        }
    }
    __syncthreads();
    */
    ...
    if( i < dimx && j <dimy && n<nc )
    {
        ...
        for(int ir = 0; ir<drad; ir++)
            for(int jr = 0; jr<drad; jr++)
            {
                ...
                nj = j-jr+r;
                if(ni<0) ni=0;
                else if(ni>dimx-1) ni= dimx-1;
                if(nj<0) nj=0;
                else if(nj>dimy-1) nj= dimy-1;
...
int main(int argc, char **argv)
{
    ...
    CUDA_CHECK;
    size_t shmBytes = (dim_x+2*rad)*(dim_y+2*rad)*(nc)*sizeof(float);
